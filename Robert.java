package org.firstinspires.ftc.teamcode.AGagne_Test;
...
public class Robert extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        //Speed Control
        if (gamepad1.a) {
            drivepower = .25; //Sets motors to quarter speed when A is pressed
        }

        if (gamepad1.b) {
            drivepower = .5; //Sets motors to half speed when B is pressed, Default speed
        }

        if (gamepad1.x) {
            drivepower = .75; //Sets motors to 75% speed when Y is pressed
        }
        ...
        double rightMotorPower = Range.clip(Math.pow(gamepad1.right_stick_y, 3), -drivepower, drivepower); //Sets Left Motor Power to Joystick
